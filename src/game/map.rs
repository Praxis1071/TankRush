...
pub enum MapSize { Small, Medium, Large, VeryLarge }

impl MapSize {
    pub const fn dimensions(self) -> (u32, u32) {
        match self { Self::Small => (24,18), Self::Medium => (32,24), Self::Large => (42,30), Self::VeryLarge => (56,40) }
    }
    const fn seed(self) -> u32 {
        match self { Self::Small => 0x1357_9BDF, Self::Medium => 0x2468_ACE1, Self::Large => 0x5A17_C0DE, Self::VeryLarge => 0x71A9_42EF }
    }
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Wall { pub min: Vec2, pub max: Vec2 }
impl Wall {
    pub const fn new(min: Vec2, max: Vec2) -> Self { Self { min, max } }
    pub fn contains(&self, point: Vec2) -> bool { point.x >= self.min.x && point.x <= self.max.x && point.y >= self.min.y && point.y <= self.max.y }
}

#[derive(Debug, Clone)]
pub struct GameMap { pub size: MapSize, pub width: f32, pub height: f32, pub walls: Vec<Wall>, pub spawn_points: Vec<Vec2> }

impl GameMap {
    pub fn rectangular(size: MapSize) -> Self { Self::generate(size) }
    ...
    pub fn generate_seeded(size: MapSize, initial_seed: u32) -> Self {
        ...
        let thickness = 14.0;
        let rooms_x = ((cells_x_total - 2) / 2) as usize;
        let rooms_y = ((cells_y_total - 2) / 2) as usize;
        ...
        while let Some(&(x,y)) = stack.last() {
            ...
            let mut count = 0;
            for &(dx,dy,direction,opposite) in &[(0isize,-1isize,0usize,2usize),(1,0,1,3),(0,1,2,0),(-1,0,3,1)] {
                ...
                if nx>=0 && ny>=0 && (nx as usize)<rooms_x && (ny as usize)<rooms_y && !visited[ny as usize*rooms_x+nx as usize] {
                    options[count]=(nx as usize,ny as usize,direction,opposite); count+=1;
                }
            }
            if count==0 { stack.pop(); continue; }
            seed=seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223);
            let (nx,ny,direction,opposite)=options[(seed as usize)%count];
            let current=y*rooms_x+x; let next=ny*rooms_x+nx;
            passages[current][direction]=true; passages[next][opposite]=true; visited[next]=true; stack.push((nx,ny));
        }
        ...
        for _ in 0..extra_openings {
            seed=seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223); let x=(seed as usize)%rooms_x;
            seed=seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223); let y=(seed as usize)%rooms_y;
            seed=seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223); let direction=(seed as usize)%4;
            let (dx,dy,opposite)=match direction {0=>(0isize,-1isize,2usize),1=>(1,0,3),2=>(0,1,0),_=>(-1,0,1)};
            let nx=x as isize+dx; let ny=y as isize+dy;
            if nx>=0 && ny>=0 && (nx as usize)<rooms_x && (ny as usize)<rooms_y {
                let current=y*rooms_x+x; let next=ny as usize*rooms_x+nx as usize;
                passages[current][direction]=true; passages[next][opposite]=true;
            }
        }

        for y in 0..rooms_y { for x in 0..rooms_x {
            let index=y*rooms_x+x; let left=16.0+x as f32*cell*2.0; let top=16.0+y as f32*cell*2.0;
            if x+1<rooms_x && !passages[index][1] { let wall_x=left+cell*2.0; walls.push(Wall::new(Vec2::new(wall_x-thickness/2.0,top),Vec2::new(wall_x+thickness/2.0,top+cell*2.0))); }
            if y+1<rooms_y && !passages[index][2] { let wall_y=top+cell*2.0; walls.push(Wall::new(Vec2::new(left,wall_y-thickness/2.0),Vec2::new(left+cell*2.0,wall_y+thickness/2.0))); }
        }}
        ...
        for index in (1..candidates.len()).rev() { seed=seed.wrapping_mul(1_664_525).wrapping_add(1_013_904_223); candidates.swap(index,(seed as usize)%(index+1)); }
    ...
    pub fn is_inside_play_area(&self, point: Vec2, radius: f32) -> bool { point.x-radius>=16.0 && point.y-radius>=16.0 && point.x+radius<=self.width-16.0 && point.y+radius<=self.height-16.0 }
