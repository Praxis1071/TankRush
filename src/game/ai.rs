...
impl Default for AiController {
    fn default() -> Self {
        Self { target: None, fire_cooldown: 0.0, think_timer: 0.0 }
    }
}
...
impl AiController {
    pub fn input(&mut self, state: &GameState, self_id: PlayerId, dt: f32) -> TankInput {
        ...
        let Some(tank) = state.tanks.iter().find(|t| t.player_id == self_id && t.alive) else { return TankInput::idle(); };
        let Some(target_id) = self.target else { return TankInput::idle(); };
        let Some(target) = state.tanks.iter().find(|t| t.player_id == target_id && t.alive) else { return TankInput::idle(); };
        ...
        TankInput { forward: distance > 180.0 || !aligned, backward: distance < 90.0 && aligned, left: delta < -0.06, right: delta > 0.06, fire: aligned && self.fire_cooldown <= 0.0 }.tap_fire(self)
    }
}

trait TapFire { fn tap_fire(self, controller: &mut AiController) -> Self; }
impl TapFire for TankInput {
    fn tap_fire(self, controller: &mut AiController) -> Self {
        if self.fire { controller.fire_cooldown = 0.45; }
        self
    }
}

fn nearest_enemy(state: &GameState, self_id: PlayerId) -> Option<PlayerId> {
    let origin = state.tanks.iter().find(|tank| tank.player_id == self_id && tank.alive)?.position;
    state.tanks.iter().filter(|tank| tank.alive && tank.player_id != self_id).map(|tank| ((tank.position - origin).length(), tank.player_id)).min_by(|a,b| a.0.total_cmp(&b.0)).map(|(_,id)| id)
}
