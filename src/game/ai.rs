...
impl AiController {
    pub fn input(&mut self, state: &GameState, self_id: PlayerId, dt: f32) -> TankInput {
        ...
        let Some(tank) = state.tanks.iter().find(|t| t.player_id == self_id && t.alive) else {
            return TankInput::idle();
        };
        let Some(target_id) = self.target else { return TankInput::idle(); };
        let Some(target) = state.tanks.iter().find(|t| t.player_id == target_id && t.alive) else {
            return TankInput::idle();
        };
...
impl TapFire for TankInput {
    fn tap_fire(mut self, controller: &mut AiController) -> Self {
